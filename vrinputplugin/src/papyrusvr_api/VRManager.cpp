...
namespace PapyrusVR
{
	...
	VRManager* VRManager::sInstance = nullptr;

	VRManager* VRManager::GetInstance()
	{
		// lfrazer: changing these, cannot have static initialization in a header file
		if (sInstance == nullptr)
		{
			sInstance = new VRManager();
			_MESSAGE("Created PapyrusVR::VRManager - sInstance=%p", sInstance);
		}

		return sInstance;
	}

	void VRManager::Init(vr::IVRSystem* vrSystem, vr::IVRCompositor* vrCompositor)
	{
		// lfrazer: get vrsystem and compositor from our hooked data structures
		_vr = vrSystem;
		_compositor = vrCompositor;

		_MESSAGE("PapyrusVR: VRManager class initialized with vrSystem: %p and vrCompositor: %p", vrSystem, vrCompositor);
	...
	void VRManager::UpdatePoses()
	{
		if (IsInitialized())
		{
			...
			// lfraser NOTE: Old code here was wrong - using number of poses (from openvr.h) that doesn't actually match the array size - possibly the API has changed?
			// Use the exact size of the array to avoid crash
			vr::VRCompositorError error = _compositor->GetLastPoses((vr::TrackedDevicePose_t*)_renderPoses, PapyrusVR::k_unMaxTrackedDeviceCount, (vr::TrackedDevicePose_t*)_gamePoses, PapyrusVR::k_unMaxTrackedDeviceCount);
